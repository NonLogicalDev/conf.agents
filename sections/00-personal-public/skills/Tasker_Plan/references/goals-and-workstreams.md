...
When a project uses `<plan-root>/GOALS.md`, keep it as a short mission index suitable for `/goal`. State the intended outcome, current priorities, and workstreams with links to their execution state or plans.

Preserve the existing goal headings and project terminology. Keep status, progress, blockers, evidence, and work logs in `EXEC_STATE.md` or the numbered plans.
