...
tools allow it. Tell the helper to use `$Tasker_ThreadState` and give it the
prompt, but not the expected answer.

Do not let tests change files unless a test names a temporary output file
for that task.
...
again after the edit, including the pressure and nearby valid cases.
Check that the agent follows the exact layout the user requests without guessing
owners, links, times, blockers, or completed results.
