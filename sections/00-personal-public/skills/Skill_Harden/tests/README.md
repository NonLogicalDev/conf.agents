...
Tell the subagent to invoke `$Skill_Harden` and give it the scenario prompt.
Do not give it the expectations or intended answer.
Do not let tests change shared files. If a test needs to write, use a
temporary directory created for that task.
