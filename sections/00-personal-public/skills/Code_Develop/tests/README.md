...
Run each scenario with a fresh isolated agent that has an empty context window. Give it the skill and the scenario prompt, but not the expectations or intended answer. Do not let tests change shared files. If a test needs to write, use a temporary directory created for that task.
